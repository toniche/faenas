...
class ThreadTestCase(TestCase):
    def setUp(self): # CREACIÓN DE LOS HILOS DE LA CONVERSACIÓN.
    ...
    def test_add_users_to_thread(self): # ASIGANCIÓN DE USUARI@S.
        self.thread.users.add(self.user1, self.user2)
        self.assertEqual(len(self.thread.users.all()), 2)

    def test_filter_thread_by_users(self): # RECUPERACIÓN HILOS A TRAVÉS DE LOS USUARI@S.
        self.thread.users.add(self.user1, self.user2)
    ...
    def test_add_messages_to_thread(self): # CREACIÓN, ASIGNACIÓN Y RECUPERACIÓN DE MENSAJES.
        self.thread.users.add(self.user1, self.user2)
        message1 = Message.objects.create(user=self.user1, content="Muy Buenas")
        message2 = Message.objects.create(user=self.user2, content="Hola Colega")
    ...
    # PARA EVITAR USAR TANTOS '.filter.filter' como en las funciones de arriba:
